...
final class Timeline: ObservableObject {
    ...
    func getJSON() {
        ...
        self.swifter.getHomeTimeline(count: 10,  success: sh, failure: nil)
    }
   ...
   func  converJSON2TweetDatas(from newTweets: [JSON]) -> [String] {
        ...
        for i in newTweets.indices {
            ...
            imageDownloader(from: tweetMedia.avatarUrlString!, setTo: IDString)()
    ...
    func imageDownloader(from urlString: String, setTo idString: String) -> () -> () {
        return{
            ...
            if let d = try? Data(contentsOf: filePath) {
                if let im = UIImage(data: d) {
                    self.tweetMedias[idString]?.avatar = im //下载好的文件赋值给image
                    print(#line, im)
                    print(#line, self.tweetMedias[idString]?.avatar)
                    print(#line, self.tweetMedias[idString]?.userName)
                }
            } else {
