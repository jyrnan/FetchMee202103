...
struct TweetsList: View {
    ...
    var tweetListType: TweetListType  //设置时间线类型
    
    var body: some View {
        return containedView() //用了一个AnyView来实现返回Metion还是Timeline的单元格的时间线
    }
            ...
            return  AnyView(
                ForEach(self.timeline.tweetIDStrings, id: \.self) {
                    ...
                    TweetRow(timeline: timeline, tweetIDString: tweetIDString, kGuardian: self.kGuardian)
                        .listRowBackground(userDefault.object(forKey: "userIDString") as? String == self.timeline.tweetMedias[tweetIDString]?.in_reply_to_user_id_str ? Color.blue.opacity(0.2) : Color.clear) //标注被提及的推文listRowBackground
                }
