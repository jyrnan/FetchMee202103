...
//                    geometry in
                    Image(uiImage: fetchMee.users[userIDString!]?.banner ?? UIImage(named: "bg")!)
                        ...
                        .overlay(LinearGradient.init(gradient: Gradient(colors: [Color(UIColor.systemBackground), Color.clear]), startPoint: .bottom, endPoint: .center))
...
//        .ignoresSafeArea()
        ...
        .navigationTitle(Text(fetchMee.users[userIDString!]?.name ?? ""))
    }
}
